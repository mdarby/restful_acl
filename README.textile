...
<pre>
  ...
  def self.is_creatable_by(user, parent_object = nil)
    # The parent_object param is automagically set to the 
    # the 'parent' of a nested routing relationship.
    
    user.accounts.include?(parent_object)
  end
</pre>
...
<%= link_to 'Create Issue', new_user_url if creatable(@account) %>
...
h3. Checking with Mom

As of v1.2, the is_creatable_by method requires two parameters, like so: <pre>def self.is_creatable_by(user, parent_object = nil)</pre>
This enables you to use a 'parent' object in a nested resource relationship.

Say that you have a User, and that user is assigned to many Projects. Further say that that User needs to create Foo under a particular Project.
Now you can do something like this:

<pre>#Foos_Controller
def self.is_creatable_by(user, parent_object = nil)
  user.projects.include?(parent_object)
end</pre>
